...
dog = Dog.create!(name: "Charlie", description: "The funniest dog in the world", birth_date: birth_date1, constraint: "Doesn't like small dogs", fun_fact: "The best swimmer you have ever seen !!",size: "Large", breed: Breed.find_by_content!("COLLIE ROUGH"), user_id: User.first.id)
...
dog = Dog.create!(name: "Molly", description: "Lovely, cute and very friendly", birth_date: birth_date2, constraint: "None", fun_fact: "She loves to wear costume",size: "Small", breed: Breed.find_by_content!("PUG"), user_id: User.second.id)
file =  URI.open("https://cdn.britannica.com/35/233235-050-8DED07E3/Pug-dog.jpg")
dog.photo.attach(io: file, filename: "dog.webp", content_type: "image/webp")

dog = Dog.create!(name: "Sam", description: "The funniest dog in the world", birth_date: birth_date1, constraint: "No constraint", fun_fact: "Loves cats", size: "Large", breed: Breed.find_by_content!("AUSTRALIAN SHEPHERD"), user_id: User.third.id)
file = URI.open("https://img.20mn.fr/2c2xoZqdQhu84Dmhb8ci9Sk/1444x920_media")
dog.photo.attach(io: file, filename: "dog.webp", content_type: "image/webp")

dog = Dog.create!(name: "Jim", description: "The funniest dog in the world", birth_date: birth_date1, constraint: "Doesn't like puppies", fun_fact: "The best toy catcher in the world!", size: "Large", breed: Breed.find_by_content!("AMERICAN STAFFORDSHIRE TERRIER"), user_id: User.third.id)
...
Walk.create!(starting_point: "45 Rue des Alouettes, 75019, Paris", duration: "30", start_time: start_time2, difficulty: "Medium", description: "A walk in les Buttes-Chaumont" , frequency: "Every other day", number_of_participant: "1", user_id: User.first.id)
